...
func (d *peerMsgHandler) HandleRaftReady() {
	...
	if result, err := d.peer.peerStorage.SaveReadyState(&ready); err != nil {
		return
	} else {
		if result != nil {
			// todo compare result region
		}
	}
...
func keyOfRequest(req *raft_cmdpb.Request) []byte {
	switch req.CmdType {
	case raft_cmdpb.CmdType_Get:
		return req.Get.Key
	case raft_cmdpb.CmdType_Put:
...
func (d *peerMsgHandler) proposeNormalRequest(msg *raft_cmdpb.RaftCmdRequest, cb *message.Callback) {
	// 获取request的key
	key := keyOfRequest(msg.Requests[0])
	// 判断key是否在region
	if key != nil {
